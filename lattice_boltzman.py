...
LATTICE_WEIGHTS = jnp.array([
    ...
    1/36, 1/36, 1/36, 1/36,     # 45 Degree veloctiies [5 ,6 ,7 ,8]
])


RIGHT_VELOCTITES = jnp.array([1, 5, 8])
LEFT_VELOCITIES = jnp.array([3, 6, 7])
UP_VELOCOTIES = jnp.array([2, 5, 6])
DOWN_VELOCTIES = jnp.array([4, 7, 8])
PURE_VERTICAL_VELOCITIES = jnp.array([0, 2, 4])
OURE_HORIZONTAL_VELOCITIES = jnp.array([0, 1, 3])
...
def get_equilibrium_discrete_velocities(macroscopic_velocities, density):
    projected_discrete_velocities = jnp.einsum(     # Einsum: Einstein Summation
        "dQ,NMd->NMQ",
        LATTICE_VELOCITIES,
...
def main():
    ...
    # Obstacle mask is an array of the shape like X or Y, but contains True if the point belongs to the obtacle and False if not.
    obstacle_mask = (
        jnp.sqrt(
            (
                X
    ...
    def update(discrete_velocities_prev):
        ...
        # 3. Apply Inflow Profile by Zou/He Dirichlet Boundary Condition on the left boundary (Zou/He scheme)
        ...
        discrete_velocities_prev =\
            discrete_velocities_prev.at[0, :, RIGHT_VELOCTITES].set(equilibrium_discrete_velocities[0, :, RIGHT_VELOCTITES])
        ...
        for i in range(N_DISCRETE_VELOCITIES):
            discrete_velocities_post_collision =\
                discrete_velocities_post_collision.at[obstacle_mask, LATTICE_INDICES[i]].set(discrete_velocities_prev[obstacle_mask, OPPOSITE_LATTICE_INDICES[i]])
    ...
    for iteration_index in tqdm(range(N_ITERATIONS)):
        ...
        if iteration_index % PLOT_EVEY_N_STEPS == 0 and VISUALIZE and iteration_index > SKIP_FIRST_N_ITERATIONS:
